<style media="screen">
.mb-3 .row .col .float-end .btn-link:hover{
	color:white;
  background-color:green;
}

.d-grid .row .col form .btn-link:hover{
	color:white;
...
<main class="container">
   ...
   <div class="col p-0 m-0">
      ...
      <div class="alert alert-success" role="alert">
         <p class="m-0 text-center">{!! \Session::get('success') !!}</p>
      </div>
      @endif
   </div>
   <div class="row">
   ...
   <div class="row row-cols-4 mt-3 mb-3">
      ...
      <div class="d-grid gap-1">
         <div class="p-2 border mb-3">
            <div class="col text-center">
               <a class="h6 p-3" href="/categories/{{$key->id}}">{{$key->name}}</a>
               <hr>
            </div>
            <div class="row">
               <div class="col">
               ...
               <div class="col">
                  <a class="mr-3 float-start" data-bs-toggle="tooltip" title="Delete the category!">
                     <form method="POST" action="{{ route('cat.destroy', $key->id) }}">
                        @csrf
                        @method('DELETE')
   ...
   <div class="modal fade close" id="edit_category_{{$key->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
      <form method="POST" action="{{ route('cat.update', $key->id)}}">
         @csrf
         @method('PUT')
